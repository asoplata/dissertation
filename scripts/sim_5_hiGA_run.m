% function data_3_pmax = sim_3_pmax_run(output_dir)
%{
sim_3_pmax_run: Simulation Run function file, that contains all
configuration required to run the trough-max/low-dose propofol simulation
in publications P16W1 and P25W1. This only calculates the simulation; it
does not plot, analyze, or save the data. For everything else, see file
"plotting_script.m"
...
% Compare PYso<-IN gGABAA 2.0 here, vs 0.1 before
% AES propo or not?
modifications  = {
    'PYso<-PYdr', 'gKNa',  1.33;
    ...
    'PYso<-IN', 'propoCondMult', 1;
    'PYso<-IN', 'propoTauMult',  1;
    'IN<-IN',   'propoCondMult', 1;
    'IN<-IN',   'propoTauMult',  1;
};
...
simulator_options={
    ...
    'plot_functions', {@dsPlot, @dsPlot, @dsPlot,...
                       @dsPlot, @dsPlot,...
                      },... % Which plot functions to call automatically
...
% -------------------------------------------------------------------
spec = assembleCortSpec(numCellsScaleFactor);
...
% -------------------------------------------------------------------
data_5_hiGA = dsSimulate(spec,'vary',vary,simulator_options{:});
